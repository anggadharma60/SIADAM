		   ...
		   if(!empty($_FILES['fileURL']['name'])) { 
			   ...
			   if ($flag == 1) {
				   for ($i = 2; $i <= $arrayCount; $i++) {
					   ...
					   $TANGGAL_PELURUSAN = $SheetDataKey['TANGGAL_PELURUSAN'];
					   $ONDESK = $SheetDataKey['ONDESK'];
					   $ONSITE = $SheetDataKey['ONSITE'];
					   $NAMA_ODP = $SheetDataKey['NAMA_ODP'];
					   $NOTE_ODP = $SheetDataKey['NOTE_ODP'];
					   $QR_ODP = $SheetDataKey['QR_ODP'];
					   $KOORDINAT_ODP = $SheetDataKey['KOORDINAT_ODP'];
					   $NAMA_OLT = $SheetDataKey['NAMA_OLT'];
					   $PORT_OLT = $SheetDataKey['PORT_OLT'];
					   $TOTAL_IN_ODP = $SheetDataKey['TOTAL_IN_ODP'];
					   $KAPASITAS = $SheetDataKey['KAPASITAS'];
					   $PORT_OUT_SPLITTER = $SheetDataKey['PORT_OUT_SPLITTER'];
					   $QR_OUT_SPLITTER = $SheetDataKey['QR_OUT_SPLITTER'];
					   $PORT_ODP= $SheetDataKey['PORT_ODP'];
					   $STATUS = $SheetDataKey['STATUS'];
					   $ONU = $SheetDataKey['ONU'];
					   $SN = $SheetDataKey['SN'];
					   $SERVICE = $SheetDataKey['SERVICE'];
					   $QR_DROPCORE = $SheetDataKey['QR_DROPCORE'];
					   $NOTE_URUT_DROPCORE = $SheetDataKey['NOTE_URUT_DROPCORE'];
					   $FLAG_OLT_PORT= $SheetDataKey['FLAG_OLT_PORT'];
					   $CONNECTIVITY_ODP_TO_OLT = $SheetDataKey['CONNECTIVITY_ODP_TO_OLT'];
					   $ODP_ONT = $SheetDataKey['ODP_ONT'];
					   $RFS = $SheetDataKey['RFS'];
					   $NOTE_HD_DAMAN = $SheetDataKey['NOTE_HD_DAMAN'];
					   $TANGGAL_UPDATE_UIM = $SheetDataKey['TANGGAL_UPDATE_UIM'];
					   $UPDATER_UIM = $SheetDataKey['UPDATER_UIM'];
					   $NOTE_QR_ODP = $SheetDataKey['NOTE_QR_ODP'];
					   $NOTE_QR_OUT_SPLITTER = $SheetDataKey['NOTE_QR_OUT_SPLITTER'];
					   $NOTE_QR_DROPCORE = $SheetDataKey['NOTE_QR_DROPCORE'];
					   $UPDATER_DAVA = $SheetDataKey['UPDATER_DAVA'];
					   ...
					   $NOTE_HD_DAMAN = filter_var(html_escape(trim($allDataInSheet[$i][$NOTE_HD_DAMAN])), FILTER_SANITIZE_STRING);
					   ...
					   $MOTE_QR_ODP = filter_var(html_escape(trim($allDataInSheet[$i][$NOTE_QR_ODP])), FILTER_SANITIZE_STRING);
					   $NOTE_QR_OUT_SPLITTER = filter_var(html_escape(trim($allDataInSheet[$i][$NOTE_QR_OUT_SPLITTER])), FILTER_SANITIZE_STRING);
					   $NOTE_QR_DROPCORE = filter_var(html_escape(trim($allDataInSheet[$i][$NOTE_QR_DROPCORE])), FILTER_SANITIZE_STRING);
